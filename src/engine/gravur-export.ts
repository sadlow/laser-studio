...
import { verbindeWege } from "./wege";

/** Ein Weg fuer den Laser – die Breite kommt vom Strahl, darum steht keine in der Datei (produktion.ts). */
...
export function gravurFuerExport(lage: Lage, e: GravurExport): { flaechen: Teil[]; pfade: GravurPfad[] } {
  ...
  if (e.art === "mittellinie") {
    // Alle Klassen zusammen: fuer den Strahl zaehlt nur der Weg. Durchgehend, ohne Doppel, Stichenden vor der
    // Kreuzung um die halbe Linienbreite gekuerzt (wege.ts).
    const linien = lage.gravur.flatMap((g) => ziehLinienAb(g.linien, material, true));
    return { flaechen: [], pfade: verbindeWege(linien, e.strahlMm / 2).map((punkte) => ({ punkte, geschlossen: false })) };
  }
  ...
  for (const g of lage.gravur) {
    ...
    if (n % 2 === 1) {
      for (const l of ziehLinienAb(g.linien, material, true)) pfade.push({ punkte: l, geschlossen: false });
    }
    ...
    for (let i = 0; i < Math.floor(n / 2); i++) {
      ...
      for (const t of teile(schneide(puffereLinien(g.linien, 2 * abstand), material), 0.0001)) {
        for (const ring of [t.aussen, ...t.loecher]) pfade.push({ punkte: ring, geschlossen: true });
      }
    }
  }
