...
async function main() {
  ...
  for (const km of kms) {
    ...
    console.log(`\n${km} km (${quelle}): format ${kz.formatfaktor.toFixed(2)} dichte ${kz.dichtefaktor.toFixed(2)} deckungVorOrt ${(kz.deckungVorOrt * 100).toFixed(1)} % netz ${(kz.netzAnteilFenster * 100).toFixed(1)} % zugefuellt ${kz.netzLoecherZugefuellt} lose ${kz.loseZurGravur}`);
